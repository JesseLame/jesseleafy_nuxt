import { ref, defineComponent, computed, watch, withAsyncContext, watchEffect, unref, mergeProps, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrInterpolate, ssrRenderList, ssrRenderClass, ssrRenderAttr } from "vue/server-renderer";
import { useRoute, useRouter } from "vue-router";
import MarkdownIt from "markdown-it";
import { u as useRecipeLanguage } from "./useRecipeLanguage-BUF9LebE.js";
import { createClient } from "@supabase/supabase-js";
import { c as useRuntimeConfig, b as useAsyncData, a as useSeoMeta, q as queryCollection } from "../server.mjs";
import "ofetch";
...
function useComments(postId) {
  ...
  async function fetchComments() {
    ...
    try {
      ...
      const flatComments = (data ?? []).filter((c) => {
        var _a, _b;
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const activeLanguage = computed(() => routeLang.value ?? "en");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (unref(recipe)) {
        ...
        if (descriptionLongHTML.value) {
          _push(`<div class="text-gray-700 mb-6 prose">${descriptionLongHTML.value ?? ""}</div>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (!ingredientsSplit.value) {
          ...
          ssrRenderList(unref(recipe).ingredients, (item, index) => {
            _push(`<li>${parseIngredient(item) ?? ""}</li>`);
          });
          _push(`<!--]--></ul>`);
        } else {
...
//# sourceMappingURL=_slug_-QEaTUETy.js.map
